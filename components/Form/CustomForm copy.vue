...
<script setup lang='ts'>
  ...
  const changeData = (value: any, key: string) => {
    ...
    // validate([key]);
    // validate();
    // console.log(formValue.value, 'formValue', formItemRef, value, key);
  };
  ...
  const validate = async (keys?: string[]): Promise<boolean> => {
    if (keys) {
      ...
      keys.forEach(async (key) => {
        if (!formItemRef.value[key]) {
          b = false;
          return;
        }
        if (props.formList.find(item => item.key === key)?.formType === 'custom') {
          if (!formCustomItemRef.value[key] || !formCustomItemRef.value[key].validate) {
            return;
          }
          const res = await formCustomItemRef.value[key].validate();
          if (!res) b = false;
          return;
        } else {
          try {
            await formItemRef.value[key].validate();
            console.log(key, 'item', formItemRef.value[key], formItemRef.value[key]);
          } catch (error) {
            b = false;
